...
const { Option } = Select;
const PedidosAprobados: React.FC = () => {
    ...
    if (selectedSucursalId && selectedSucursalId > 0) {
      try {
        ...
        const pedidosFiltrados = todosLosPedidos.filter(
          (pedido: Pedido) => pedido.estado === Estado.CONFIRMADO
        );
        setPedidos(pedidosFiltrados);
      } catch (error) {
        console.error("Error al cargar los pedidos aprobados:", error);
        message.error("Error al cargar los pedidos aprobados");
      }
    }
  };
  ...
  const handleEstadoChange = async (id: number, nuevoEstado: Estado) => {
    try {
      ...
      cargarPedidosAprobados(); // Recargar los pedidos para reflejar el cambio de estado
    } catch (error: any) {
      ...
      render: (_text: any, record: any) => (
        <Button
          onClick={() => {
            ...
            // No necesitas cambiar el estado aquí, solo guarda el ID del pedido seleccionado
            // La lógica para cambiar el estado se manejará en el modal
    ...
    <div>
      ...
      >
        <Select
          ...
          style={{ width: "100%" }}
        >
          <Select.Option value="EN_PREPARACION">EN PREPARACION</Select.Option>
...
export default PedidosAprobados;
